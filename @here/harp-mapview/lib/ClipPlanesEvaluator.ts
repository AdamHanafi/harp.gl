...
export class TiltViewClipPlanesEvaluator extends TopViewClipPlanesEvaluator {
    ...
    protected evaluateDistanceSphericalProj(
        ...
        if (camera instanceof THREE.PerspectiveCamera) {
            ...
            // Ratio of the height of the camera compared to the distance between the center of the
            ...
            const height = 1 / Math.tan(halfVerticalFovAngle);

            // Screen space direction vector pointing to the top right frustum point from the camera

            const upperRightDirection = this.m_tmpVectors[1];
            upperRightDirection.set(camera.aspect, 1, -height);
